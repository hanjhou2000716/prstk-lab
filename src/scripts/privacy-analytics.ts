...
const getSafeProperties = properties => {
  const safe = {};
...
const bind = () => {
  ...
  const details = document.getElementById('privacy-settings');
  ...
  document.getElementById('tool-search')?.addEventListener('input', event => { const hasQuery = Boolean(event.target.value.trim()); if (hasQuery && !searchStarted) track('search_started'); searchStarted = hasQuery; });
  document.addEventListener('click', event => { const target = event.target.closest?.('[data-analytics-event]'); if (!target) return; track(target.dataset.analyticsEvent, { category: target.dataset.analyticsCategory }); });
};
